...
def parse_credentials(fileIn, fileOut):
    ...
    out_file = open(fileOut, 'w', encoding="utf-8")
    ...
    for index in range(0, int(file_len / 0x84)):
        ...
        if password.startswith(b'C = ') or password.startswith(b'DC = '):
            continue
        ...
        username = username[0:userlen]
        password = password[0:passlen]

        fset = frozenset({"username": username, "password": password}.items())
        if fset not in seen:
            
            data = "%s,%s,%s\n" % (safe_decode(username), safe_decode(password), status)
            seen.add(fset)
